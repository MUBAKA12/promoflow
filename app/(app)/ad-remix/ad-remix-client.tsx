...
import {
  createCreative,
  createProductForCreative,
  generateHooksForCreative,
  addCustomHook,
  deleteHook,
  generateVideosFromHooks,
} from "./actions";
import { Upload, Sparkles, Loader2, Trash2, Plus, Film, Check } from "lucide-react";
...
export default function AdRemixClient({
  channels,
  products,
  ...
  return (
    <div className="p-10 max-w-5xl">
      <h1 className="font-display text-2xl font-semibold mb-1">Ad Remix</h1>
      <p className="text-[var(--text-muted)] mb-6">
        Upload a video you already have, attach a product, and generate new hooks — same
        footage, new opening line each time.
      </p>
        ...
        <div>
          {selected ? (
            ...
            <div className="text-center py-24 text-[var(--text-muted)] text-sm rounded-xl border border-dashed border-[var(--border)]">
              Upload or select a creative to generate hooks for it.
            </div>
          )}
        </div>
      </div>
...
function UploadCreative({
  ...
  async function handleUpload(files: FileList | null) {
    ...
    try {
      ...
      if (productMode === "existing" && productId) {
        ...
        finalProductId = await createProductForCreative({
          channelId,
      ...
      const path = `${user.id}/${channelId}/ad-remix/${Date.now()}-${file.name}`;
      const { error: uploadError } = await supabase.storage
        .from("ad-remix-creatives")
        .upload(path, file);
  ...
  return (
    <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4 space-y-3">
      <p className="text-xs font-medium text-[var(--text-muted)]">New creative</p>
      <input
        value={name}
        onChange={(e) => setName(e.target.value)}
        placeholder="Creative name (optional — we'll use the filename if blank)"
...
function CreativeDetail({
  ...
  async function handleGenerateHooks() {
    ...
    setGenerating(true);
    try {
      const result = await generateHooksForCreative(creative.id, creative.product_id, count);
      if (!result.ok) {
  ...
  async function handleAddCustom() {
    ...
    const id = await addCustomHook(creative.id, customHook.trim(), customCaption.trim());
    onHooksAdded([
      {
        id,
        creative_id: creative.id,
  ...
  return (
    <div>
      <div className="flex gap-4 mb-6">
        <div className="w-32 aspect-[9/16] rounded-lg overflow-hidden bg-[var(--surface-raised)] shrink-0">
          {url && <video src={url} className="w-full h-full object-cover" muted controls />}
        </div>
        ...
        <div className="space-y-2 mb-6">
          {hooks.map((h) => (
            ...
            >
              ...
              <button
                onClick={() => deleteHook(h.id).then(() => onHookDeleted(h.id))}
      ...
      {batchDone ? (
        ...
        >
          {batchSubmitting ? <Loader2 size={16} className="animate-spin" /> : <Film size={16} />}
